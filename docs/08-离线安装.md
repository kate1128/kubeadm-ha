...
- 下载所需离线包
  ```
  curl -LO https://files.saas.hand-china.com/kubeadm-ha/kubeadm-ha-1.25.12-amd64.tar
...
- 准备 kubeadm-ha(ansible) 环境
  ...
  # 加载 kubeadm-ha 镜像
  docker load -i kubeadm-ha-1.25.12-amd64.tar
  ```
...
- 运行 kubeadm-ha 镜像
  ...
  docker run -d --restart=always --name kubeadm-ha \
    ...
    setzero/kubeadm-ha:1.25.12-amd64
  ```
